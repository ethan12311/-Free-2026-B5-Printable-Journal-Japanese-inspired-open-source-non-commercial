# GitHub Repository Description (paste into the "About" field on your repo)

## Short Description (350 chars max — for GitHub About field)

A free, open-source B5 printable yearly journal for 2026. 92 pages with Japanese-inspired frontispiece (一期一會), monthly calendars (Mon-first), weekly writing pages, and 15 guided reflection prompts. Strictly Cormorant Garamond serif. Non-commercial CC BY-NC 4.0. Completely prompted and powered by GLM 5.2.

## Even Shorter (for Twitter/social media)

Free 2026 B5 printable journal: Japanese-inspired frontispiece, monthly calendars, weekly writing pages, 15 reflection prompts. All Cormorant Garamond serif. Non-commercial. Built with GLM 5.2. 📓✨

## Tags for GitHub Topics (add in repo settings)

```
journal planner printable pdf b5 bullet-journal hobonichi jibun-techo japanese-stationery 2026 minimal reflection gratitude-journal analog paper cormorant-garamond ai-assisted glm non-commercial open-source creative-commons
```

## SEO Keywords (for the README meta tags, if you add a GitHub Pages site)

```
printable journal, B5 planner, 2026 journal, free planner template, bullet journal printable, Hobonichi alternative, Takahashi Shoten style, Japanese stationery, reflection prompts, gratitude journal, open source planner, CC BY-NC, GLM 5.2, AI-assisted design, Cormorant Garamond, Monday-first calendar, weekly writing pages, monthly summary prompts, Ichigo Ichie, 一期一会
```

## Sample Social Media Post

> 📓 Just published: 2026 Journal — a free, open-source B5 printable yearly journal inspired by Hobonichi Techo & Takahashi Shoten. 92 pages, Cormorant Garamond serif throughout, Japanese frontispiece (一期一會), 15 reflection prompts. Non-commercial CC BY-NC 4.0. Completely prompted & powered by GLM 5.2. Print it, write in it, share it. → [link]

## [Free] 2026 B5 Printable Journal — Japanese-inspired, open-source, non-commercial**
>
> Hi everyone! I made a 92-page B5 printable yearly journal for 2026, inspired by Hobonichi Techo and Takahashi Shoten. It's completely free, open-source, and non-commercial (CC BY-NC 4.0).
>
> **Features:**
> - Japanese frontispiece with 一期一會 (Ichigo Ichie)
> - 12 monthly calendar pages (Monday-first, CJK+English weekday labels)
> - 4-5 weekly writing pages per month (each week = one full B5 page)
> - 15 guided reflection prompts (monthly summaries + year-end batch)
> - Strictly Cormorant Garamond serif — zero "AI taste"
> - B5 (176×250mm), print-ready
>
> **Tech note:** The journal was completely prompted and powered by GLM 5.2 (Z.ai's AI), with human art direction. All design decisions (font choice, layout, prompt wording) were human-driven.
>
> Download: [GitHub link]
>
> Would love feedback! Especially on the reflection prompts and the frontispiece phrase choice.
